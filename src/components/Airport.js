...
export default function ListAirport() {
  ...
  const removeFlightFromAirport = (airportCode, flightNumber) => {
    AirportService.removeFlightFromAirport(airportCode, flightNumber)
      .then((res) => {
        console.log(airportCode, flightNumber);
        getAllAirports();
        navigate("/airports")
      })
      .catch((error) => console.log(error));
                  ...
                  <tr key={flight.id}>
                    ...
                    <Link to={`/edit-flight?flightNumber=${flight.flightNumber}`}  style={{ marginRight: "30px" }} className="btn btn-info">Edit</Link>
                      <button
                        ...
                        onClick={() => removeFlightFromAirport(airport.code, flight.flightNumber)}
